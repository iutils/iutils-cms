...
<header class="am-topbar am-topbar-fixed-top">
    <div class="am-container">
        <h1 class="am-topbar-brand" style="padding:0;margin-top: 9px;">
            <a href="${pageContext.request.contextPath}/"><img class="logo" src="${site.logo}" /></a>
        </h1>
        ...
        <div class="am-collapse am-topbar-collapse" id="collapse-head">
            ...
            <form id="searchForm" class="am-topbar-form am-topbar-right" action="${ctxF}/${empty category?0:category.id}/search${urlSuffix}<c:if test="${not empty param.id}">?id=${param.id}</c:if>" method="get" style="margin:0;padding:0;">
                ...
                <div class=" am-topbar-left user-bar-info">
                    ...
                    <shiro:user>欢迎您！<a href="${ctxF}/personal${urlSuffix}<c:if test="${not empty param.id}">?id=${param.id}</c:if>" title="个人中心"><shiro:principal/></a> <a class="am-btn-link" href="${ctx}/logout">注销</a></shiro:user>
                </div>
...
<c:set var="user" value="${fnc:getLoginUser()}"></c:set>
<script>
    //是否已登录
